...
public class Solution11 {
	 ...
	 *	
	 * @category Problem: What is the greatest product of four adjacent numbers in the same direction 
	...
	public static void main(String[] args){
		...
		for(int col = 0; col < inputSquare.length; col++){
			for(int row = 0; row < inputSquare.length; row++){
				double tmpProd;
				
				//Vertikal
				if(row < inputSquare.length - NMBR){
					tmpProd = inputSquare[col][row];
					for(int i=1; i < NMBR; i++){
						tmpProd *= inputSquare[col][row + i];
					}
					product = Math.max(product, tmpProd);
				}
				
				
				//Horizontal
				if(col < inputSquare.length - 4){
					tmpProd = inputSquare[col][row];
					for(int i=1; i < NMBR; i++){
						tmpProd *= inputSquare[col + i][row];
					}
					product = Math.max(product, tmpProd);
				}
				
				//Diagonal 	aufw�rts/forw�rts
				if((col < inputSquare.length - NMBR) && (row >= NMBR)){
					tmpProd = inputSquare[col][row];
					for(int i = 1; i < NMBR; i++){
						tmpProd *= inputSquare[col + i][row - i];
					}
					product = Math.max(product, tmpProd);
				}
				
				//Diagonal abw�rts/forw�rts
				if((row < inputSquare.length - NMBR) && (col < inputSquare.length - NMBR)){
					tmpProd = inputSquare[col][row];
					for(int i = 1; i < NMBR; i++){
						tmpProd *= inputSquare[col + i][row + i];
					}
